...
func (c *MPIJobController) getOrCreateWorker(mpiJob *kubeflow.MPIJob) ([]*corev1.Pod, error) {
	...
	for i := 0; i < int(*worker.Replicas); i++ {
		...
		if errors.IsNotFound(err) {
			worker := newWorker(mpiJob, i, c.gangSchedulerName)
...
func newWorker(mpiJob *kubeflow.MPIJob, index int, gangSchedulerName string) *corev1.Pod {
	...
	setupSSHOnPod(&podTemplate.Spec, mpiJob)
...
func setupSSHOnPod(podSpec *corev1.PodSpec, job *kubeflow.MPIJob) {
