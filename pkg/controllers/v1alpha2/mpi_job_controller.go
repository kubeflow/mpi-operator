...
func (c *MPIJobController) updateMPIJobStatus(mpiJob *kubeflow.MPIJob, launcher *batchv1.Job, worker *appsv1.StatefulSet) error {
	...
	launcherType := kubeflow.ReplicaType(kubeflow.MPIReplicaTypeLauncher)
	if launcher != nil {
		now := metav1.Now()
		if launcher.Status.Active > 0 {
			mpiJobCopy.Status.ReplicaStatuses[launcherType].Active += mpiJobCopy.Status.ReplicaStatuses[launcherType].Active
			if mpiJobCopy.Status.StartTime == nil {
				mpiJobCopy.Status.StartTime = &now
			}
		} else if launcher.Status.Succeeded > 0 {
			mpiJobCopy.Status.ReplicaStatuses[launcherType].Succeeded += mpiJobCopy.Status.ReplicaStatuses[launcherType].Succeeded
			if mpiJobCopy.Status.CompletionTime == nil {
				mpiJobCopy.Status.CompletionTime = &now
			}
		} else if launcher.Status.Failed > 0 {
			mpiJobCopy.Status.ReplicaStatuses[launcherType].Failed += mpiJobCopy.Status.ReplicaStatuses[launcherType].Failed
		}
	}
	workerType := kubeflow.ReplicaType(kubeflow.MPIReplicaTypeWorker)
	if worker != nil {
		now := metav1.Now()
		if launcher.Status.Active > 0 {
			mpiJobCopy.Status.ReplicaStatuses[workerType].Active += mpiJobCopy.Status.ReplicaStatuses[workerType].Active
			if mpiJobCopy.Status.StartTime == nil {
				mpiJobCopy.Status.StartTime = &now
			}
		} else if launcher.Status.Succeeded > 0 {
			mpiJobCopy.Status.ReplicaStatuses[workerType].Succeeded += mpiJobCopy.Status.ReplicaStatuses[workerType].Succeeded
			if mpiJobCopy.Status.CompletionTime == nil {
				mpiJobCopy.Status.CompletionTime = &now
			}
		} else if launcher.Status.Failed > 0 {
			mpiJobCopy.Status.ReplicaStatuses[workerType].Failed += mpiJobCopy.Status.ReplicaStatuses[workerType].Failed
		}
	}
